...
class MeetingAttendance(BaseModel):
    ...
    attended = models.BooleanField(
        default=False, db_index=True, verbose_name="Presente"
    )
    absence_reason = models.TextField(
        blank=True, verbose_name="Motivo da Ausência"
    )
    ...
    def clean(self):
        ...
        if self.member_id and self.cell_meeting_id:
            ...
            is_cell_member = CellMember.objects.filter(
                member=self.member, cell=self.cell_meeting.cell, is_active=True
            ).exists()
